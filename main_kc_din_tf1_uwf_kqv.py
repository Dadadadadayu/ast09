...
fh = logging.FileHandler(
        '2021-10-9-rms-ktm-din-addkc-kqv.log')
...
from rms_ktm_kc_din_uwf_kqv import tf, FM, set_seed
import numpy as np
...
def build(d_model, rate, num_skills, kc_length):
    ...
    for i, col in enumerate(SPARSE_FEATURES[2:]):
        ...
        if col == 'item_id':
            ...
            cur_user.append(single_input_embed_2)
            inputs_embed_2.append(single_input_embed_2)
    list_inputs = []
    # ------ list ------
    ...
    cur_u = tf.concat(cur_user, axis=1)

    fm = FM(rate, d_model, num_skills, kc_length)

    output = fm(inputs_embed_2, inputs[:2], cur_kc, cur_kc_mask, cur_p, cur_u, True)
    predict_output = fm(inputs_embed_2, inputs[:2], cur_kc, cur_kc_mask, cur_p, cur_u, False)
