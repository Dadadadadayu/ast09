...
fh = logging.FileHandler(
    '2021-9-20-rms-ktm-din-addkc.log')
...
from rms_ktm_kc_din_test import tf, FM, set_seed
import numpy as np
...
def build(d_model, rate, num_skills, kc_length):
    ...
    output, _, _, _ = fm(inputs_embed_2, inputs[:2], cur_kc, cur_kc_mask, cur_p, True)
    predict_output, sim, din, master = fm(inputs_embed_2, inputs[:2], cur_kc, cur_kc_mask, cur_p, False)
    ...
    pred_model = Model(inputs=inputs+list_inputs, outputs=[predict_output, sim, din, master])
...
test_skill = []
test_skill.append([[0] * num_skills for i in range(num_skills)])
test_skill.append([[0] * num_skills for i in range(num_skills)])
test_skill.append([[0] for i in range(num_skills)])
test_skill.append([[0] for i in range(num_skills)])
test_skill.append([[i, 0, 0, 0] for i in range(num_skills)])
test_skill.append([[1, 0, 0, 0] for i in range(num_skills)])

list_gauc, list_auc, list_acc = [], [], []
for epoch in range(100):
    model.fit(train, [Y_train], batch_size=4096, epochs=1, verbose=1, shuffle=True)
    test_pred, _, _, _ = pred_model.predict(test)
    ...
    if epoch == 99:
        print('-------begin to calculate pattern prob----------------')
        pred_, sim, din, master = pred_model.predict(train)
        num_example = len(Y_train)
        pred = np.reshape(pred_, (num_example,))

        print('-------begin to store csv file----------------')
        tmp = pd.DataFrame()
        tmp['is_correct'] = Train_data['Y']
        tmp['stu_id'] = Train_data['X_user']
        tmp['problem_id'] = Train_data['X_item']
        tmp['skill_id'] = Train_data['X_skill']
        tmp['skill_mask'] = Train_data['X_skill_mask']
        tmp['pred'] = pred
        tmp['master_prob'] = [item for item in master]
        tmp.to_csv('rms_ktm_din.csv', index=False)
        print('-------store csv file successfully----------------')

        print('-------begin to store txt file----------------')
        _, all_sim, _, _ = pred_model.predict(test_skill)
        import numpy as np

        tt = []
        for item in all_sim:
            tt.append(item[:num_skills])
        # 保存
        np.savetxt("skill_relation.txt", tt)
        print('-------store txt file successfully----------------')
