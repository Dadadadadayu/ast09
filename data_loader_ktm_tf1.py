...
class Data_loader(object):
    def __init__(self):
        self.assist09 = pd.read_csv("assist2009.csv", encoding="latin1", index_col=False)

    def data_load(self):
        assist09 = self.assist09
        # Filter out users that have less than min_interactions interactions
        assist09 = assist09.groupby("user_id").filter(lambda x: len(x) >= 3)
        ...
        for user, item_id, correct, skill_ids in zip(assist09['user_id'], assist09['item_id'],
            ...
            sub_wins_nums + [0] * len_, sub_fails_nums + [0] * len_, [float(correct)]])
        return dic, num_users, num_items, num_skills
