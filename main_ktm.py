...
def build(d_model, rate, num_skills, kc_length):
    ...
    list_embeddings_2 = []
    for col in LIST_FEATURES:
        list_embeddings_1.append(Embedding(feature_config[col], 1))  # skill
        list_embeddings_2.append(Embedding(feature_config[col], d_model))

        list_embeddings_1.append(Embedding(feature_config[col], 1))  # wins
        list_embeddings_2.append(Embedding(feature_config[col], d_model))

        list_embeddings_1.append(Embedding(feature_config[col], 1))  # fails
        list_embeddings_2.append(Embedding(feature_config[col], d_model))
    ...
    for i, col in enumerate(LIST_FEATURES):
        ...
        wins_input_embed_1 = list_embeddings_1[i + 1](single_input)  # batch_size*1*kc_*1
        wins_input_embed_2 = list_embeddings_2[i + 1](single_input)  # batch_size*1*kc_*d_model

        fails_input_embed_1 = list_embeddings_1[i + 2](single_input)  # batch_size*1*kc_*1
        fails_input_embed_2 = list_embeddings_2[i + 2](single_input)  # batch_size*1*kc_*d_model
        ...
        # skill embeddings
        inputs_embed_1.append(tf.reduce_sum(tf.multiply(tf.reduce_sum(single_input_embed_1, 1),
                                          tf.reshape(mask_input, [-1, kc_length, 1])), 1, keep_dims=True))
        inputs_embed_2.append(tf.reduce_sum(tf.multiply(tf.reduce_sum(single_input_embed_2, 1),
                                          tf.reshape(mask_input, [-1, kc_length, 1])), 1, keep_dims=True))

        # wins embeddings
        inputs_embed_1.append(tf.reduce_sum(tf.multiply(tf.multiply(tf.reduce_sum(wins_input_embed_1, 1),
                                                                    tf.reshape(mask_input, [-1, kc_length, 1])),
                                          tf.reshape(inputs[0], [-1, kc_length, 1])), 1, keep_dims=True))
        inputs_embed_2.append(tf.reduce_sum(tf.multiply(tf.multiply(tf.reduce_sum(wins_input_embed_2, 1),
                                                        tf.reshape(mask_input, [-1, kc_length, 1])),
                                          tf.reshape(inputs[0], [-1, kc_length, 1])), 1, keep_dims=True))

        # fails embeddings
        inputs_embed_1.append(tf.reduce_sum(tf.multiply(tf.multiply(tf.reduce_sum(fails_input_embed_1, 1),
                                                                    tf.reshape(mask_input, [-1, kc_length, 1])),
                                                        tf.reshape(inputs[1], [-1, kc_length, 1])), 1, keep_dims=True))
        inputs_embed_2.append(tf.reduce_sum(tf.multiply(tf.multiply(tf.reduce_sum(fails_input_embed_2, 1),
                                                                    tf.reshape(mask_input, [-1, kc_length, 1])),
                                                        tf.reshape(inputs[1], [-1, kc_length, 1])), 1, keep_dims=True))
    ...
    output = fm(inputs_embed_1, inputs_embed_2, True)
    predict_output = fm(inputs_embed_1, inputs_embed_2, False)
...
for epoch in range(100):
    model.fit(train_seq[1:]+train_list_seq, train_seq[0:1], batch_size=4096, epochs=1, verbose=1)
