...
from rms_ktm_kc import tf, FM, set_seed
import numpy as np
...
def build(d_model, rate, num_skills, kc_length):
    ...
    fm = FM(rate, d_model, num_skills)

    output = fm(inputs_embed_2, inputs[:2], cur_kc, cur_kc_mask, True)
    predict_output = fm(inputs_embed_2, inputs[:2], cur_kc, cur_kc_mask, False)
